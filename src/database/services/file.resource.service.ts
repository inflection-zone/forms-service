...
import { FileResourceSearchResults, FileResourceSearchFilters } from '../../domain.types/file.resource.upload.domain.types';
...
export class FileResourceService {
    ...
    upload = async (domainModel: FileResourceUploadDomainModel): Promise<FileResourceDto> => {
        ...
        if (domainModel.IsMultiResolutionImage === true) {
            if (!this.isSupportedImageType(domainModel.FileMetadata.MimeType)) {
                ...
                console.log('Multi-resoulution image type is not supportd!');
            }
            ...
            for await (var versionMetadata of imageVersions) {
                ...
                var url =
                    ConfigurationManager.BaseUrl() +
                    '/api/v1/file-resources/' +
                    versionMetadata.ResourceId +
    ...
    uploadLocal = async (sourceLocation: string, storageLocation: string, isPublicResource: boolean): Promise<FileResourceDto> => {
    ...
    replaceLocal = async (
        ...
        if (existingStorageKey) {
            ...
            console.log(`Deleted resource: ${deleted ? 'Yes' : 'No'}`);
        }
    ...
    searchAndDownload = async (filters: FileResourceSearchFilters): Promise<string> => {
    ...
    delete = async (id: string): Promise<boolean> => {
        ...
        for await (var version of versions) {
            ...
            if (!deleted) {
                throw new AppError('Error deleting version for resource!', 422);
            }
        }
    ...
    private async uploadFileToStorage(fileMetadata: FileResourceMetadata) {
        var dateFolder = TimeUtils.getDateString(new Date(), DateStringFormat.YYYY_MM_DD);
    ...
    private async cleanupDirectories(parentFolder) {
        try {
            ...
            var cleanupBeforeInMinutes = ConfigurationManager.TemporaryFolderCleanupEvery();

            var cleanupBefore = TimeUtils.subtractDuration(new Date(), cleanupBeforeInMinutes, DurationType.Minute);
            ...
            for await (var d of directories) {
                ...
                for await (var c of directoryContents) {
                    ...
                    var isBefore = TimeUtils.isBefore(createdDateTime, cleanupBefore);
                    if (isBefore) {
    ...
    private isSupportedImageType = (mimeType: string): boolean => {
        var supportedTypes = ['image/bmp', 'image/jpeg', 'image/png'];
    ...
    private generateImageVersions = async (domainModel: FileResourceUploadDomainModel): Promise<FileResourceMetadata[]> => {
        ...
        await sharp(sourceFilePath).resize(thumbnailWidth, thumbnailHeight).toFile(thumbnailFilename);
        ...
        await sharp(sourceFilePath).resize(previewWidth, previewHeight).toFile(previewFilename);
