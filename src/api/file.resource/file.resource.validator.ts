...
import { FileResourceUploadDomainModel, FileResourceMetadata, FileResourceSearchFilters } from '../../domain.types/file.resource.upload.domain.types';
...
import { DownloadDisposition } from '../../domain.types/file.resource.upload.domain.types';
...
export class FileResourceValidator extends BaseValidator {
    ...
    getUploadDomainModel = (request: express.Request): FileResourceUploadDomainModel[] => {
        ...
        var currentUserId = request.body.UserId;
    ...
    upload = async (request: express.Request): Promise<FileResourceUploadDomainModel[]> => {
        // if (!request.files) {
        //     ErrorHandler.handleValidationError('No file uploaded!!');
        // }
    ...
    getByVersionName = async (request: express.Request): Promise<FileResourceMetadata> => {
        await param('id').exists().escape().isUUID().run(request);

        await param('version').exists().trim().run(request);

        await query('disposition').optional().trim().run(request);
    ...
    getByVersionId = async (request: express.Request): Promise<FileResourceMetadata> => {
        ...
        await param('versionId').exists().trim().isUUID().run(request);
    ...
    private async searchQueryParams(request) {
        await query('ownerUserId').optional().trim().escape().isUUID().run(request);

        await query('isPublicResource').optional().trim().escape().isBoolean().run(request);

        await query('version').optional().trim().escape().run(request);

        await query('referenceId').optional().trim().isUUID().escape().run(request);

        await query('referenceType').optional().trim().escape().run(request);

        await query('referenceKeyword').optional().trim().escape().run(request);

        await query('tag').optional().trim().escape().run(request);
        ...
        await query('itemsPerPage').optional().trim().escape().isInt().run(request);
    ...
    private async validateBody(request) {
        await body('FileName').optional().trim().escape().run(request);

        await body('OwnerUserId').optional().trim().escape().run(request);

        await body('IsPublicResource').optional().trim().escape().run(request);

        await body('IsMultiResolutionImage').optional().trim().escape().run(request);

        await body('References').optional().run(request);

        await body('Tags').optional().run(request);
    ...
    private getFilter(request): FileResourceSearchFilters {
        const pageIndex = request.query.pageIndex ? parseInt(request.query.pageIndex as string, 10) : 0;

        const itemsPerPage = request.query.itemsPerPage ? parseInt(request.query.itemsPerPage as string, 10) : 25;
